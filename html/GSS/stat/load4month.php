...
///// CLASSIC
$rqt=$bdd->prepare("select count(id_op) AS t_op_classic from operation o , utilisateurs u where o.id_user = u.id_user and month(dat_crea)='$monthsys' and year(dat_crea)='$yearsys' and u.privilege in('gest');");
$rqt->execute();
...
//TP
$rqt2=$bdd->prepare("select count(id_dem) AS t_op_tp from demande where  month(date_tdem)='$monthsys' and year(date_tdem)='$yearsys' and id_user not in (13) and et_dem not in (3,5);");
$rqt2->execute();
...
FROM ( select *
       ...
       inner join ( SELECT o.id_user as id_user2,count(o.id_op)as nb
                   ...
                   where o.id_user=u.id_user and  month(o.dat_crea)='$monthsys' and year(o.dat_crea)='$yearsys'
                   group by o.id_user
                   )as res
...
FROM (select *
      ...
      inner join ( SELECT d.id_atp as id_user2,count(d.id_dem)as nb
                  ...
                  where d.id_atp=a.id_user and  month(d.date_tdem)='$monthsys' and year(d.date_tdem)='$yearsys' and d.id_user not in (13) and d.et_dem not in (3,5)
                  group by d.id_atp
                )as res
      on u1.id_user=res.id_user2
      )as r
